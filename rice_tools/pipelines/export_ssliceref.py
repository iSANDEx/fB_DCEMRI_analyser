...
Example:
    >>> python -m rice_tools.pipelines.export_ssliceref -c PATH_TO_CONFIG
"""
...
from ..io import load_dicom_folder, load_patient_data, build_mask
...
def main(config_structure):
    ...
    # Load ROI information from the info folder:
    ...
    for patient_id in patientsID:
        ...
        if patient_id in config_structure['dicom']['patients_id']:
            ...
            df_filter = {'PatientID': patient_id}
            # Load the mask
            for visit_nro in config_structure['dicom']['visits_id']:
                ...
                if visit_nro in patient_dict['Visits'].keys():
                    ...
                    df_filter['VisitID']=int(visit_id)
                    ...
                    # Create a mask where labels are the anatomies in the sslice_ref dataframe:
                    img_mask = build_mask(sslice_ref_df, df_filter)
                    ...
                    if img_mask:
                        ...
                        # Segment to plot: 
                        # 0: desc. aorta; 1: tumour; 2: asc. aorta
                        for segment_label in img_mask['labels'].keys():
                            # segment_label = 'tumour'
                            segment_index = img_mask['labels'][segment_label]
                                ...
                                if patient_dict['Visits'][visit_nro]['Sequences'][sequence]:
                                    ...
                                    mfig = overlay_mask(patient_dict['Visits'][visit_nro]['Sequences'][sequence]['nparray'], 
                                                        ...
                                                        savepath = os.path.join(outputfolder, 
                                                                                f'roi_segments_{sequence}_{patient_id}_{visit_id}_{segment_label}.png')
                                    )
                                    ...
                                    if si_bline.empty:
                                        ...
                                        bline_index = si_bline.loc[si_bline["Anatomy"].apply(lambda x: SequenceMatcher(None, segment_label, x).ratio()).idxmax(), "BaseLine"] # si_bline['BaseLine'].max()
                                    ...
                                    roi_df = extract_roi(patient_dict['Visits'][visit_nro]['Sequences'][sequence]['nparray'],
                                                         ...
                                                         segment_index=segment_index, # 0: desc. aorta; 1: tumour; 2: asc. aorta
                                    ...
                                    if not roi_df.empty:
                                        ...
                                        roi_df['ROI label'] = segment_label

                                        logger.debug(roi_df.head())
                                        roi_df.to_csv(os.path.join(outputfolder, f'sslice_roi_{sequence}_{segment_label}.csv'))
