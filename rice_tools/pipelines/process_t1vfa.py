...
Example:
    >>> python -m rice_tools.pipelines.process_t1vfa -c PATH_TO_CONFIG
"""
...
from ..utils import validate_path, setup_logging, extract_t1vfa
from ..io import read_csv, plot_t1vfa, plot_timeseries
...
def main(config_structure):
    ...
    tseries_stats = {'xaxis': 'FA',
                     'grouping_index': 'time_index',
                     'yaxis': 'median',
                     'fill_area': 'stddev'
                     }
    tseries_label = {'xlabel': 'Flip Angle [degree]',
                     'ylabel': 'Signal Average'
                     }
    title_sufix = 'SI'
    ...
    # TODO: Refactor for T1VFA
    # Loop over the patients and visits to get the (time, values) pairs
    # The main difference is that I need to combine two sequences (or three): FA5, FA15 (and 4D(ti=2))
    for patient in config_structure['dicom']['patients_id']:
        ...
        for sequence in config_struct['dicom']['sequence_patterns']:
            ...
            for visit in config_structure['dicom']['visits_id']:
                ...
                tseries_data = extract_t1vfa(all_data_df, df_filter, 
